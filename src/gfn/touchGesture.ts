...
/**
 * Reconstructs trackpad-style two-finger gestures (right-click, scroll) on
 * top of GFN's absolute-touch input path, which -- unlike MouseTrackpadZone's
 * relative mode -- only ever has one finger's worth of cursor position to
 * work with (see GfnStreamAdapter.queuePointerInput / NVST's absolute mouse
 * message). NativeTouchOverlay already reports every finger's own
 * pointerdown/move/up individually rather than batched, so this tracks
 * currently-down pointers across calls the same way MouseTrackpadZone reads
 * React Native's touches array, just event-by-event instead of all at once.
 */
export class GfnTouchGestureTracker {
  ...
  private onDown(event: GfnPointerEvent): void {
    ...
    if (this.order.length === 2 && !this.primaryCommitted) {
      // Second finger joined before the first committed -- a two-finger
      // gesture, not a click/drag. Cancel the pending commit so the primary
      // never warps the cursor or presses the left button at all.
    ...
    // A third+ finger, or a second finger joining after the primary already
    // committed (mid-drag) -- out of scope, left untouched.
  }
  ...
  private onUp(event: GfnPointerEvent): void {
    ...
    if (wasPrimary) {
      ...
      if (this.primaryCommitted) {
        ...
        this.callbacks.buttonUp(MOUSE_LEFT);
      }
      this.primaryId = null;
      this.primaryCommitted = false;
    }
    ...
    if (wasTwoFingerPointer) {
      ...
      if (!otherStillDown) {
        if (this.twoFinger && !this.twoFingerMoved) {
          ...
          // a right-click, at wherever the cursor already was.
  ...
  private commitPrimary(pos?: PointerPos): void {
    ...
    this.callbacks.buttonDown(MOUSE_LEFT);
  }
  ...
  // the stream ending). Still releases a committed left button so it can't
  // get stuck down on the server side.
  dispose(): void {
    if (this.primaryCommitted) {
      this.callbacks.buttonUp(MOUSE_LEFT);
    }
