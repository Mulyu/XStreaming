...
7. **Confirm no divergence from the architecture.** After the pass's item(s) are done, re-check the result against `.claude/knowledge/architecture.md` -- layer placement, slice isolation (no same-layer slice importing another directly), segment split, public API -- rather than assuming moving files was enough. If anything still diverges (an import that had to reach across slices, a segment that doesn't cleanly fit ui/model/api/lib/config, a missing barrel), say so explicitly instead of calling the pass done, and treat it as a new item for a future pass rather than silently accepting it.
